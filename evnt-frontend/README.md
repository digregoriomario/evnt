...
Nota: usare Node LTS 20/22/24. Con Node 25 Expo puo fallire nella selezione porta locale.

### UI kit

La base UI dell'app e in `src/components/ui`, con primitive stile Gluestack (`GluestackUIProvider`, `Button`, `Card`, `Badge`, `Input`, `Box`, `Text`). Il setup ufficiale Gluestack v3 via CLI richiede piu spazio disco di quello disponibile in questa macchina al momento.

Per aprirla su dispositivo o simulatore:
...
Il backend REST (Node + Express + TypeScript, Prisma, PostgreSQL + PostGIS) si
trova nella cartella `../evnt-backend`. Implementa l'intero modello E-R con
autenticazione JWT, eventi con feed geolocalizzato, iscrizioni, preferiti,
